...
local DIR = os.getenv("HOME") .. "/.config/awesome/widgets/pacman-widget/"
...
local function worker(user_args)
	...
	function pacman_widget.set(new_value)
		...
		pacman_widget
			...
			:set_image(ICON_DIR .. (tonumber(new_value) > 0 and "pacman" or "pacman-full") .. ".svg")
	end
	...
	pacman_widget:buttons(awful.util.table.join(awful.button({}, 1, function()
	...
	timer = select(
		2,
		awful.widget.watch([[bash -c "checkupdates 2>/dev/null"]], _config.interval, function(widget, stdout)
			local upgrades_tbl = {}
			...
			local header = wibox.widget({
				{
					...
					{
						markup = "<b>"
							.. (upgrading and "Upgrading " .. #upgrades_tbl .. " Packages" or (#upgrades_tbl == 0 and "No" or #upgrades_tbl) .. " Available Upgrades")
							.. "</b>",
			...
			for k, v in ipairs(upgrades_tbl) do
				...
				function row.get_txt()
					return row:get_children_by_id("txt")[1].text
				end
				function row.set_idx(idx)
					row:get_children_by_id("idx")[1]:set_text(idx)
				end
...
return setmetatable(pacman_widget, {
	__call = function(_, ...)
		return worker(...)
	end,
})
